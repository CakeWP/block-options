...
class Edit extends Component {
	...
	render() {
		...
		const { value, onChange, isActive, colors } = this.props;
		...
		if (activeColorFormat) {
			...
			if (styleColor) {
				activeColor = styleColor.replace(
			...
			if (currentClass) {
				...
				activeColor = getColorObjectByAttributeValues(
					colors,
					colorSlug
				).color;
			}
								...
								<ColorPalette
									...
									onChange={(color) => {
										if (color) {
											...
											if (
												typeof window.editorskitInfo !==
													'undefined' &&
												window.editorskitInfo.supports
													.color_palette
											) {
												colorObject =
													getColorObjectByColorValue(
	...
	withSelect((select) => {
		...
		return {
			...
			isDisabled: select('core/edit-post').isFeatureActive(
				'disableEditorsKitHighlightFormats'
			),
		};
	}),
