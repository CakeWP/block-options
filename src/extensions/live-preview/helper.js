...
export async function updatePreview() {
	...
	const { autosave, savePost } = dispatch('core/editor' );

	// Checking if the current post is draft.
	const isDraft =
		[ 'draft', 'auto-draft' ].indexOf(getEditedPostAttribute('status' ) ) !==
		-1;
...
export function reloadPreview() {
	const currentPreviewReference =
		select('editorskit/preview').getCurrentPreviewRef();
