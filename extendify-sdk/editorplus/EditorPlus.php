...
if ( ! class_exists( 'edpl__EditorPlus' ) ) {
	...
	final class ExtendifySdkEditorPlus {
		...
		public function __construct() {
			if ( $this->isSupported() ) {
				...
				add_action( 'wp_head', array( $this, 'enqueueStylesheet' ) );
				...
				$this->templates = array( 'editorplus-template.php' => 'Extendify Template' );
				add_filter(
					'body_class',
					function ( $classes ) {
				...
				$postTypes         = get_post_types( array( '_builtin' => false ), 'names', 'and' );
				$postTypes['post'] = 'post';
	...
	add_action( 'after_setup_theme', array( 'ExtendifySdkEditorPlus', 'getInstance' ) );
}//end if
