...
function (_Component) {
  ...
  _babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_1___default()(ReadingTime, [{
    ...
    value: function updateMeta() {
      ...
      var wasAutosavingPost = select('core/editor').isAutosavingPost(); // First remove any existing subscription in order to prevent multiple saves

      if (!!saveUnsubscribe) {
        saveUnsubscribe();
      }

      saveUnsubscribe = subscribe(function () {
        ...
        var shouldTriggerSave = wasSavingPost && !isSavingPost && !wasAutosavingPost; // Save current state for next inspection.
